...
import src.lora_VIT.lora_psi as lora_psi
import subprocess
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Pytorch psi training for vgg of imagenet"
    )
    ...
    if "vit" in args.net_name.lower():
        ...
        model, low_rank_layers = vit_b32_dlrt(
            r=args.init_r, n_classes=n_classes, adapter_name="psi"
        )

        model = model.to(device)
        print(model)
        print(low_rank_layers)
        params = [
            p
            for n, p in model.named_parameters()
            if p.requires_grad == True
            and  ("lora_e" in n.lower() or "psi" in n.lower())
        ]
        # for n, p in model.named_parameters():
    ...
    t = lora_psi.LoraTrainer(model, low_rank_layers, train_loader, val_loader)
