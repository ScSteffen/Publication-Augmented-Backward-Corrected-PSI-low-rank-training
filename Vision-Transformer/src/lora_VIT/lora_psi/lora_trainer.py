...
class LoraTrainer:
    ...
    def train_classic(
        ...
        if args.wandb == 1:
            ...
            wandb.init(
                project="PSI_Paper_benchmark{}".format(
        ...
        for epoch in range(num_epochs + 2):
            ...
            print("loss, epoch ", loss.detach().cpu().numpy())
    ...
    def run_classic_integrator(
        ...
        with tqdm(self.train_loader, desc="Training", unit="batch") as pbar:
            for batch_idx, (data, targets) in enumerate(pbar):
                ...
                ## 1) Deactivate all gradients.
                # for name, param in self.model.named_parameters():
                #    if name in active_param_names:
                #        param.requires_grad = False
                #
                # 1) Deactivate L gradients

                if batch_idx % substeps == 0:
                    for l in self.dlrt_layers:
                        l.deactivate_L_grads()
                        l.activate_K_grads()

                if batch_idx % substeps < substeps / 2:
                    ...
                    with torch.amp.autocast("cuda"):
                        ...
                        optimizer.step()  # K step integration

                if batch_idx % substeps == int(substeps / 2):
                    for l in self.dlrt_layers:
                        l.step(mode="K", adapter_name="psi")

                    # ---- L step forward pass ----

                    # 1) Deactivate K gradients
                    for l in self.dlrt_layers:
                        l.deactivate_K_grads()
                        l.activate_L_grads()

                if substeps / 2 < batch_idx % substeps < substeps - 1:
                    # 2) Forward pass
                    with torch.amp.autocast("cuda"):
                        out = self.model(data)
                        out = out.logits if hasattr(out, "logits") else out
                    ...
                    optimizer.step()  # L step integration

                if batch_idx % substeps == substeps - 1 or batch_idx == len(pbar) - 1:
                    for l in self.dlrt_layers:
                        l.step(mode="L", adapter_name="psi")
    ...
    def train_budget(
        ...
        if args.wandb == 1:
            ...
            wandb.init(
                project="Adalora_benchmark{}".format(args.dataset_name, args.net_name),
                name=watermark,
            )
            wandb.config.update(args)
            wandb.watch(self.model)
