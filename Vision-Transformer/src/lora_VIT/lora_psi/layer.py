...
class AdaLoraLayer(LoraLayer):
    ...
    def __init__(self, base_layer: nn.Module) -> None:
        ...
        # self.U0 = nn.ParameterDict({})
        self.lora_psi_K = nn.ParameterDict({})
        self.lora_psi_L = nn.ParameterDict({})
    ...
    def activate_K_grads(self):
        self.lora_psi_K["psi"].requires_grad = True
        self.lora_E["psi"].requires_grad = False

        self.mode = "K"

    def deactivate_K_grads(self):
        self.lora_psi_K["psi"].requires_grad = False

    def activate_L_grads(self):
        self.lora_psi_L["psi"].requires_grad = True
        self.lora_E["psi"].requires_grad = False

        self.mode = "L"

    def activate_S_grads(self):
        self.lora_psi_L["psi"].requires_grad = False
        self.lora_psi_K["psi"].requires_grad = False
        self.lora_E["psi"].requires_grad = True

        self.mode = "S"

    def deactivate_L_grads(self):
        self.lora_psi_L["psi"].requires_grad = False
    ...
    def update_layer(
        ...
        self.rmax = min([r, self.in_features, self.out_features])
        ...
        self.lora_E[adapter_name] = nn.Parameter(
            torch.zeros(r, r, dtype=torch.float32), requires_grad=False
        )
        ...
        self.lora_psi_K[adapter_name] = 0.0 * nn.Parameter(
            self.lora_A[adapter_name][:, :r] @ self.lora_E[adapter_name][:r, :r],
            requires_grad=True,
        )
        self.lora_psi_L[adapter_name] = nn.Parameter(
            self.lora_B[adapter_name][:, :r] @ self.lora_E[adapter_name][:r, :r].T,
            requires_grad=True,
        )
    ...
    def reset_lora_parameters(self, adapter_name):
        if adapter_name in self.lora_A.keys():
            ...
            self.lora_psi_K[adapter_name] = torch.nn.Parameter(
                self.lora_A[adapter_name] @ self.lora_E[adapter_name],
                requires_grad=True,
            )

            self.lora_psi_L[adapter_name] = torch.nn.Parameter(
                self.lora_B[adapter_name] @ self.lora_E[adapter_name].T * 0.0,
                requires_grad=True,
            )

    @torch.no_grad()
    def step(self, mode, adapter_name="psi"):
        ...
        K_view = self.lora_psi_K[adapter_name]
        L_view = self.lora_psi_L[adapter_name]

        # perform K-step
        if mode == "K":
            r = self.r[adapter_name]
            rx2 = min(2 * r, self.rmax)
            U_hat = torch.linalg.qr(
                torch.cat((U_view[:, :r], K_view[:, :r]), 1), "reduced"
            )[0][:, :rx2]
            # perform S-step
            S_bar = U_hat.T @ U_view[:, :r] @ S_view[:r, :r]
            # print(S_bar.shape)
            # L step preparation
            L_view[:, :rx2] = V_view[:, :r] @ S_bar.T
            U_view[:, :rx2] = U_hat
            self.r[adapter_name] = rx2  # update rank
        # perform L-step
        elif mode == "L":
            r = self.r[adapter_name]
            tmp_qr = torch.linalg.qr(L_view[:, :r], "reduced")
            V_view[:, :r] = tmp_qr[0][:, :r]
            S_view[:r, :r] = tmp_qr[1][:r, :r].T

        else:
            print("Wrong step defined: ", mode)

    @torch.no_grad()
    def Truncate(self, tau=1e-1, adapter_name="psi"):
        ...
        # update s
        S_view[:r1, :r1].copy_(torch.diag(d[:r1]))

        # update u and v
        U_view[:, :r1] = U_view[:, :r] @ P[:, :r1]
        V_view[:, :r1] = V_view[:, :r] @ Q.T[:, :r1]

        K_view[:, :r1] = U_view[:, :r1] @ S_view[:r1, :r1]

        self.r[adapter_name] = int(r1)
        self.mode = "K"  # For evaluation during training

    @torch.no_grad()
    def get_singular_values(self, adapter_name="psi"):
        r = self.r[adapter_name]
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                psi_K = self.lora_psi_K[active_adapter]
                psi_L = self.lora_psi_L[active_adapter]

                if self.mode == "K":
                    result += (dropout(x) @ psi_K[:, :r]) @ lora_B[:, :r].T
                elif self.mode == "L":
                    result += (dropout(x) @ lora_A[:, :r]) @ psi_L[:, :r].T
                elif self.mode == "S":
