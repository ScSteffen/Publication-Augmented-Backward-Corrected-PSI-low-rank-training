...
def create_layer(layer):
    ...
    if layer['type'] == 'GeoLoRA':
        return GeoLoRALayer(layer['dims'][0], layer['dims'][1], layer['rank'], layer['tol'])
...
#################################################################################################################################################
### Define GeoLoRA layer ########################################################################################################################
#################################################################################################################################################
class GeoLoRALayer(nn.Module):
    def __init__(self, input_size, output_size, rank, tol=1e-2, init_compression=0.5):
        """Constructs a low-rank layer of the form U*S*V'*x + b, where 
           U, S, V represent the facorized weight W and b is the bias vector
        Args:
            input_size: input dimension of weight W
        ...
        """
        # construct parent class nn.Module
        super(GeoLoRALayer, self).__init__()

        # set rank and trunction tolerance
        self.tol = tol

        self.r = rank
        self.rmax = int(min(input_size, output_size) / 2) - 1
        rmax = self.rmax

        # initializes factorized weight
        self.U = nn.Parameter(torch.randn(input_size, rmax))
        self.V = nn.Parameter(torch.randn(output_size, rmax))
        ...
        self.U1 = nn.Parameter(torch.randn(input_size, rmax))
        self.V1 = nn.Parameter(torch.randn(output_size, rmax))
        self.U1.requires_grad = False
        self.V1.requires_grad = False

        self.singular_values, _ = torch.sort(torch.randn(rmax) ** 2, descending=True)
    ...
    @torch.no_grad()
    def step(self, learning_rate, mode=0):
        """Performs a steepest descend training update on specified low-rank factors
        Args:
            learning_rate: learning rate for training
        """
